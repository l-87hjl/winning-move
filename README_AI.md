# AI Agent Documentation Standards — Winning Move

This file directs AI agents (Claude Code, Codex, and any future automation) on
how to document and maintain changes to this project. Following these standards
keeps the repository auditable and maintainable across multiple automated
sessions.

> **Claude Code users:** See `CLAUDE.md` for Claude Code–specific behaviour
> overrides, hook guidance, and tool-call conventions that supplement this
> document.
...
docs/
  ...
  graphics-spec.md       Image/asset dimension and format conventions.
CLAUDE.md                Claude Code–specific agent instructions.
README_AI.md             This file — general AI agent documentation standards.
...
- **Time scale per era** (`timeScale` property in each ERA_PRESET): governs how
  many real-world months each turn represents and what the starting calendar date
  is. WW2 eras use months; Cold War eras use quarters; modern eras use months;
  AI-emergence scenarios use years.
...
- Continent victory requires **3 or more** controlled continents so small
  ...
  for an instant win.
- `detectDeadlock()` in `game.js` should not trigger before turn 30 and
  only when >60% of regions are neutral/contested — lower thresholds cause
  frustratingly short games.

---

## 6. Tile development and rebellion resource discipline

Tile development levels (0 = undeveloped → 3 = industrial) affect resource
yield. When adding upgrade logic:
- Development cost scales with current level; upgrading tier 2→3 costs more
  than 0→1.
- Conversion between resource types (e.g., forest → farmland) incurs a
  redevelopment penalty: development resets by at least one tier and
  instability spikes temporarily.
- Contested regions with active revolutions: the **controlling faction**
  does not receive resource income from those tiles. The **revolution
  instigator** may draw a portion of that income as "local rebellion yield."
- If no faction controls a contested tile, the resources are lost (burned
  in conflict) unless a `rebellionYieldFraction` is set for the scenario.
...
- [ ] `detectDeadlock()` turn threshold ≥ 30 and region fraction > 0.60.
...
- [ ] Each new era has a `timeScale` and `startDate` defined in ERA_PRESETS.
- [ ] Tile development levels are initialised on all regions in `startGame()`.
- [ ] Rebellion yield deducts from controlling faction and credits instigator.
- [ ] Graphics assets follow the spec in `docs/graphics-spec.md`.
